...
@section('content')
    <section class="appointment-content-area padding-top-120 padding-bottom-90">
        <div class="container">
                    ...
                    <div class="top-search-wrapper">
                        <div class="right-part">
                            <select name="category" class="form-control">
                              ...
                              @foreach($all_price_plan as $category)
                                    <option @if($category->id == $cat_id) selected @endif value="{{$category->id}}">{{$category->title}}</option>
                                @endforeach 
                            </select>
            ...
            <div class="row">
                        @forelse($all_users as $data)
                        <div class="col-lg-3">
                           <div class="card members-single-item">
                            <div class="cat">
                                <a href="">{{$data->package_name}}</a>
                            </div>   
                            @php
                            $user_img = get_attachment_image_by_id($data->image,null,true);
                            @endphp
                             @if (!empty($user_img))
                             <img  src="{{$user_img['img_url']}}" alt="Card image cap">
                             @endif  
                           <div class="body">
                            <h4 class="m-3"><a href="{{route('frontend.members.single',$data->id)}}">{{$data->name}}</a></h4>
                           </div>
