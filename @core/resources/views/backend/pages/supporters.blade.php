...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="table-wrap table-responsive">
                            ...
                            <tbody>
                            @foreach($all_supporters as $data)
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        @php
                                            $supporter_img = get_attachment_image_by_id($data->image,null,true);
                                            $img_url = '';
                                        @endphp
                                        @if (!empty($supporter_img))
                                            <div class="attachment-preview">
                                                <div class="thumbnail">
                                                    <div class="centered">
                                                        <img class="avatar user-thumb" src="{{$supporter_img['img_url']}}" alt="">
                                                    </div>
                                                </div>
                                            </div>
                                            @php  $img_url = $supporter_img['img_url']; @endphp
                                        @endif
                                    </td>
                                    <td>
                                        <x-delete-popover :url="route('admin.supporters.delete',$data->id)"/>
                                        <a href="#"
                                           data-toggle="modal"
                                           data-target="#supporter_item_edit_modal"
                                           class="btn btn-xs btn-primary btn-sm mb-3 mr-1 supporter_edit_btn"
                                           data-id="{{$data->id}}"
            ...
            <div class="col-lg-6 mt-5">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.supporters')}}" method="post" enctype="multipart/form-data">
                            @csrf
    ...
    <div class="modal fade" id="supporter_item_edit_modal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('admin.supporters.update')}}" id="supporter_edit_modal_form"  method="post" enctype="multipart/form-data">
                    <div class="modal-body">
                        @csrf
                        <input type="hidden" class="form-control" name="id"  id="supporter_id" >
                        <div class="form-group">
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','#bulk_delete_btn',function (e) {
                ...
                if(allIds != '' && bulkOption == 'delete'){
                    ...
                    $.ajax({
                        'type' : "POST",
                        'url' : "{{route('admin.supporters.bulk.action')}}",
                        'data' : {
                            _token: "{{csrf_token()}}",
                            ids: allIds
                        },
            ...
            $(document).on('click','.supporter_edit_btn',function(){
                var el = $(this);
                var id = el.data('id');
                var title = el.data('title');
                var form = $('#supporter_edit_modal_form');
                var image = el.data('image');
                var imageid = el.data('imageid');

                form.find('#supporter_id').val(id);
