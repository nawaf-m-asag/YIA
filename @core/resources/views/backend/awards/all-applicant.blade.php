                                        ...
                                        <table id="all_user_table" >
                                            ...
                                            @foreach($all_applicant as $data)
                                                <tr>
                                                    ...
                                                    <td>{{$data->id}}</td>
                                                    <td>{{!empty($data->award) ? optional($data->award)->title : __('This award is not available or delete')}}</td>
                                                    <td>{{!empty($data->award) ? optional($data->award)->position : __('This award is not available or delete')}}</td>
                                                    <td>{{date_format($data->created_at,'d M Y h:i:sA')}}</td>
                                                    @php
                                                        $all_data = [
                                                                'award_title' => !empty($data->award) ? $data->award->title : __('This award is not available or delete'),
                                                    ...
                                                    <td>
                                                         <x-delete-popover :url="route('admin.awards.applicant.delete',$data->id)"/>
                                                        <a href="#"
                                                           data-toggle="modal"
    ...
    <div class="modal fade" id="send_mail_modal" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form action="{{route('admin.awards.applicant.mail')}}" method="post" enctype="multipart/form-data">
                    @csrf
        ...
        $(document).ready(function() {
            ...
            $(document).on('click','#bulk_delete_btn',function (e) {
                ...
                if(allIds != '' && bulkOption == 'delete'){
                    ...
                    $.ajax({
                        'type' : "POST",
                        'url' : "{{route('admin.awards.applicant.bulk.delete')}}",
                        'data' : {
                            _token: "{{csrf_token()}}",
                            ids: allIds
                        },
