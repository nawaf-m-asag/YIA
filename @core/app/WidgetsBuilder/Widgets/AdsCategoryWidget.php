...
class AdsCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $ads_posts = AdsCategory::where(['status' => 'publish','lang' => LanguageHelper::user_lang_slug()])->orderBy('id','desc')->get();
        if (!empty($post_items)){
            $ads_posts = $ads_posts->take($post_items);
        }
        ...
        foreach ($ads_posts as $post) {
            $output .= '<li><a href="' . route('frontend.ads.category', ['id' => $post->id,'any' => Str::slug(purify_html($post->name))]) . '"> <i class="fas fa-chevron-'.$icon_class.'"></i>' . purify_html($post->name) . '</a></li>';
        }
        $output .= '</ul>';
        $output .= '</div>';
